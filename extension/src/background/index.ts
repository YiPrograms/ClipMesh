...
class ClipMeshService {
  ...
  async requestServerPermission(serverUrl:string):Promise<boolean>{const url=validatedServerUrl(serverUrl);return chrome.permissions.request({origins:[`${url.origin}/*`]});}
  ...
  async reloadClipboard():Promise<void>{const response=await chrome.runtime.sendMessage({target:'offscreen',command:'read'});if(response?.error)throw new Error(response.error);if(response)this.current=deserializeItem(response);}
  ...
  private async writeClipboard(item:NormalizedClipboardItem):Promise<void>{const hash=await contentHash(item);this.localSuppress={hash,at:Date.now()};const response=await chrome.runtime.sendMessage({target:'offscreen',command:'write',item:serializeItem(item)});if(response?.error)throw new Error(response.error);this.current=item;}
